...
from .align.aligner import get_align_mat
from .align.landmark import FACIAL_LANDMARKS_IDXS

class Align(object):
    def extract(self, image, lmark, size=256, align_eyes=False):
        alignment = get_align_mat(lmark, size, align_eyes)
        extracted = self.transform(image, alignment, size, 48)
        return extracted, alignment

    def transform(self, image, mat, size=256, padding=0):
        matrix = mat * (size - 2 * padding)
        matrix[:,2] += padding
        return cv2.warpAffine(image, matrix, (size, size))

    def transform_points(self, points, mat, size, padding=48):
        matrix = mat * (size - 2 * padding)
        matrix[:,2] += padding
